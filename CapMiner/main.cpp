...
bool g_exitOnError = false;  // Whether or not CapMiner should exit on mining threads errors
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("CapMiner - GPU ProgPOW(0.9.3) miner for Zing");
    ...
    void help()
    {
        cout << "CapMiner - GPU ProgPOW(0.9.3) miner for Zing" << endl
             << "minimal usage : CapMiner [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default CapMiner will try to use all devices types" << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    CapMiner provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes CapMiner can stay" << endl
                 ...
                 << "    --exit              FLAG Stop CapMiner whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how CapMiner chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects CapMiner passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    CapMiner restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'CapMiner -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When CapMiner reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode CapMiner tries to auto-detect the correct" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = CapMiner_get_buildinfo();
    cout << endl
         << endl;
    std::cout << R"(             
          ,clllllllllllllc;' ':llc:;,''        
...
((/ __|((_)_ ((_)_\ |  \/  | (_) _(_/( (_))   ((_) 
 | (__ / _` || '_ \)| |\/| | | || ' \))/ -_) | '_| 
  \___|\__,_|| .__/ |_|  |_| |_||_||_| \___| |_|   
             |_|                                   

    )" << '\n';
    cout << "CapMiner " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'CapMiner --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try CapMiner --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
